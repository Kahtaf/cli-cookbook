...
export default class InventoryDeleteItem extends Command {
  static description = 'Deletes an item from an inventory, if it exists'

  static examples = [
    'ccb inventory delete-item myFridge --item=eggs',
  ]
  ...
  public async run(): Promise<void> {
    const {args, flags} = await this.parse(InventoryDeleteItem)
